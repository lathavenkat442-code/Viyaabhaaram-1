...
const App: React.FC = () => {
  ...
  const saveStock = (itemData: Omit<StockItem, 'id' | 'lastUpdated' | 'history'>, id?: string) => {
    const sanitizedVariants = itemData.variants.map(v => ({
        ...v,
        sizeStocks: v.sizeStocks || [{ size: 'General', quantity: 0 }]
    }));
    
    // Logic to update state and record history
    if (id) {
      setStocks(prev => prev.map(s => {
        if (s.id === id) {
            ...
            const oldQty = s.variants ? s.variants.reduce((acc, v) => acc + v.sizeStocks.reduce((sum, ss) => sum + ss.quantity, 0), 0) : 0;
            ...
            if (oldQty !== newQty) {
                ...
                newHistory.unshift({
                    ...
                    change: `${oldQty} ➔ ${newQty} (${sign}${diff})`
                });
                actionAdded = true;
            }

            // Generic update if no specific action logged but something changed in details
            if (!actionAdded) {
                 if (s.name !== itemData.name || s.category !== itemData.category || JSON.stringify(s.variants) !== JSON.stringify(sanitizedVariants)) {
                    newHistory.unshift({
                        date: Date.now(),
                        action: 'UPDATED',
      ...
      const newHistory: StockHistory[] = [{
          ...
          change: `Initial Stock: ${initialQty}`
      }];
...
const AddStockModal: React.FC<{ onSave: (item: Omit<StockItem, 'id' | 'lastUpdated' | 'history'>, id?: string) => void; onClose: () => void; initialData?: StockItem; language: 'ta' | 'en'; t: any }> = ({ onSave, onClose, initialData, language, t }) => {
    // ... [Content of AddStockModal kept same] ...
    // NOTE: For brevity in this XML, I'm assuming the existing content of AddStockModal is preserved here. 
    // Since I am only supposed to return files that need to be updated, and App.tsx contains multiple components.
    // I will include the full content of AddStockModal to be safe.
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
       <div className="bg-white w-full max-w-sm rounded-[2rem] p-6 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
             ...
             {/* Party Name - New Field for Ledger */}
             <div className="space-y-1">
                ...
                <div className="relative">
                   ...
                   <input 
                      ...
                      className="w-full bg-gray-50 p-4 pl-12 rounded-2xl font-bold text-gray-900 outline-none focus:ring-2 focus:ring-indigo-100" 
                      placeholder="e.g. Raja, Kumar Stores" 
                   />
                </div>
             </div>
